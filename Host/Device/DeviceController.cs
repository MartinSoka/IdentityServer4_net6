...
using IdentityServer4.Host.Consent;
...
namespace IdentityServer4.Host.Device
{
    [Authorize]
    [SecurityHeaders]
    public class DeviceController : Controller
    {
        ...
        private async Task<ProcessConsentResult> ProcessConsent(DeviceAuthorizationInputModel model)
        {
            ...
            if (model.Button == "no")
            {
                ...
                // emit event
                await _events.RaiseAsync(new ConsentDeniedEvent(User.GetSubjectId(), request.Client.ClientId, request.Client.AllowedScopes.Select(a => a)));
            }
            ...
            else if (model.Button == "yes")
            {
                ...
                if (model.ScopesConsented != null && model.ScopesConsented.Any())
                {
                    ...
                    // emit event
                    await _events.RaiseAsync(new ConsentGrantedEvent(User.GetSubjectId(), request.Client.ClientId, request.Client.AllowedScopes.Select(a => a), grantedConsent.ScopesValuesConsented, grantedConsent.RememberConsent));
                }
                else
                {
        ...
        private async Task<DeviceAuthorizationViewModel> BuildViewModelAsync(string userCode, DeviceAuthorizationInputModel model = null)
        {
            ...
            if (request != null)
            {
                ...
                if (client != null)
                {
                    var resources = await _resourceStore.FindEnabledResourcesByScopeAsync(request.Client.AllowedScopes.Select(a => a));
                    ...
                    {
                        _logger.LogError("No scopes matching: {0}", request.Client.AllowedScopes.Select(a=>a));
                    }
                }
                else
                {
